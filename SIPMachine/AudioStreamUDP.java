...
public class AudioStreamUDP {
	...
	public static final boolean DEBUG = GlobalSettings.DEBUG_UDP;
...
class Receiver implements Runnable {
	...
	public void run() {
		...
		try {
			...
			int bufferLengthInFrames = line.getBufferSize()
					/ AudioStreamUDP.BUFFER_VS_FRAMES_RATIO;
			int bufferLengthInBytes = bufferLengthInFrames * frameSizeInBytes;
			if (AudioStreamUDP.DEBUG) {
				System.out.println("bufferLengthInFrames = "
						+ bufferLengthInFrames);
			...
			while (receiverThread != null) {
				...
				if (remoteHost.equals(packet.getAddress())) {
					...
					if (AudioStreamUDP.DEBUG) {
						System.out.println("Received bytes = " + numBytesRead
								+ ", packets = " + packets++);
					}
			...
			// If the thread is blocked in a receive call, an exception is
			// thrown when
	...
	/**
	 * Thanks to: Paulo Levi. Lines can fail to open because they are already in
	 * use. Java sound uses OSS and some linuxes are using pulseaudio. OSS needs
	 * exclusive access to the line, and pulse audio highjacks it. Try to open
	 * another line.
	 * 
	 * @param format
	 * @return a open line
	 * @throws IllegalStateException
	 *             if it can't open a dataline for the audioformat.
	 */
	private SourceDataLine getSourceDataLine(AudioFormat format) {
		...
		try {
			...
			throw new IllegalStateException("Error trying to aquire dataline.",
					e);
		}
		if (audioException == null) {
			...
			throw new IllegalStateException(
					"Couldn't aquire a dataline, probably "
							+ "because all are in use. Last exception:",
					audioException);
		}
	}
}
	...
	public void run() {
		...
		try {
			...
			while (senderThread != null) {
				...
				packet = new DatagramPacket(data, numBytesRead, remoteAddress,
						remotePort);
				socket.send(packet);
				if (AudioStreamUDP.DEBUG) {
					System.out.println("Bytes sent = " + numBytesRead
							+ ", packets = " + packets++);
				}
			}
		} catch (SocketException se) {
